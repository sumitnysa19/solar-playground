...
BasisTextureLoader.prototype = Object.assign( Object.create( Loader.prototype ), {
	...
	detectSupport: function ( renderer ) {
		...
		var hasExt = function( name ) {
			var gl = (typeof renderer.getContext === 'function') ? renderer.getContext() : null;
			if ( gl && gl.getExtension ) {
				// Query WebGL extension directly to avoid Three.js console warnings
				return !!gl.getExtension( name );
			}
			// Fallbacks if context not available yet
			if ( renderer.extensions && typeof renderer.extensions.has === 'function' ) {
				try { return renderer.extensions.has( name ); } catch (e) { return false; }
			}
			if ( renderer.extensions && typeof renderer.extensions.get === 'function' ) {
				try { return !!renderer.extensions.get( name ); } catch (e) { return false; }
			}
			return false;
		};

		config.astcSupported = hasExt( 'WEBGL_compressed_texture_astc' );
		config.bptcSupported = hasExt( 'EXT_texture_compression_bptc' );
		config.etcSupported = hasExt( 'WEBGL_compressed_texture_etc1' );
		config.dxtSupported = hasExt( 'WEBGL_compressed_texture_s3tc' );
		config.pvrtcSupported = hasExt( 'WEBGL_compressed_texture_pvrtc' )
			|| hasExt( 'WEBKIT_WEBGL_compressed_texture_pvrtc' );
